...
public class DashboardService {
    ...
    public DashboardService(FeedbackRepository feedbackRepository, FeedbackAnalysisRepository analysisRepository, ResponseDraftRepository draftRepository) {
    ...
    public DashboardSummary summary() {
        EnumMap<FeedbackStatus,Long> statuses=new EnumMap<>(FeedbackStatus.class);
        for(var s:FeedbackStatus.values()) statuses.put(s,feedbackRepository.countByStatus(s));
        EnumMap<Sentiment,Long> sentiments=new EnumMap<>(Sentiment.class);
        for(var s:Sentiment.values()) sentiments.put(s,analysisRepository.countBySentiment(s));
        EnumMap<SupportCategory,Long> categories=new EnumMap<>(SupportCategory.class);
        for(var c:SupportCategory.values()) categories.put(c,analysisRepository.countByCategory(c));
        EnumMap<Urgency,Long> urgencies=new EnumMap<>(Urgency.class);
        for(var u:Urgency.values()) urgencies.put(u,analysisRepository.countByUrgency(u));
