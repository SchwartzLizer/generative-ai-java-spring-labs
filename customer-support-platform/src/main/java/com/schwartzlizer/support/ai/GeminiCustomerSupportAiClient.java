...
public final class GeminiCustomerSupportAiClient implements CustomerSupportAiClient {
    private final ChatCompletionGateway gateway; private final ObjectMapper objectMapper; private final AnalysisPromptFactory analysisPrompts; private final ResponsePromptFactory responsePrompts;
    public GeminiCustomerSupportAiClient(ChatCompletionGateway gateway, ObjectMapper objectMapper, AnalysisPromptFactory analysisPrompts, ResponsePromptFactory responsePrompts) { this.gateway=Objects.requireNonNull(gateway); this.objectMapper=Objects.requireNonNull(objectMapper); this.analysisPrompts=Objects.requireNonNull(analysisPrompts); this.responsePrompts=Objects.requireNonNull(responsePrompts); }
    @Override public FeedbackAnalysisResult analyze(String message) {
        try {
            String raw=gateway.complete(analysisPrompts.create(message)); if (raw == null || raw.isBlank()) throw new AiProviderException("AI provider returned an empty response");
